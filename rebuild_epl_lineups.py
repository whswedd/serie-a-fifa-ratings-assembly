...
session.headers.update({
    "User-Agent": "Mozilla/5.0 (compatible; EPL-Lineup-Rebuilder/1.1)",
...
def as_list(x):
    ...
    if isinstance(x, dict):
        for k in ("data", "content", "matches", "items"):
...
def next_cursor(x):
    ...
    if not isinstance(pg, dict):
        return None
    return pg.get("_next") or pg.get("next")
...
def player_name(p):
    return (
        ...
        " ".join(filter(None, [
            pget(p, "player", "firstName"),
            pget(p, "player", "lastName")
        ])).strip()
    )
...
def fetch_season_matches(season_id):
    ...
    while True:
        ...
        data = get_json("/api/v2/matches", params)
        batch = as_list(data)
        print(f"season {season_id}: page {page}, returned {len(batch)} matches", flush=True)
        ...
        new_cursor = next_cursor(data)
        if not new_cursor:
            break
        if new_cursor == cursor:
            raise RuntimeError("Pagination returned the same _next cursor twice.")
        if added == 0:
            raise RuntimeError("Pagination produced no new match IDs.")

        cursor = new_cursor
        page += 1
        time.sleep(0.15)

    matches.sort(key=lambda m: str(m.get("kickoff") or m.get("kickoffTime") or ""))

    if len(matches) < 300:
        raise RuntimeError(
            f"Only {len(matches)} unique matches returned for season {season_id}"
        )
    return matches
...
for season_label, season_id in SEASONS.items():
    ...
    print("unique season matches:", len(matches), flush=True)
    ...
    for n, m in enumerate(matches, 1):
        ...
        for side, team, ps in [
            ...
            for i, p in enumerate(ps, 1):
                all_rows.append({
                    ...
                    "RawJSON": json.dumps(
                        p, ensure_ascii=False, separators=(",", ":")
                    ),
                })
        ...
        coverage.append({
            ...
            "HomeStarters": len(hs),
            "AwayStarters": len(aw),
            "CompleteXI": len(hs) == 11 and len(aw) == 11,
            "EndpointVersion": used_version,
            "Error": err if lineups is None else None,
        })

        if n % 20 == 0 or n == len(matches):
            pd.DataFrame(all_rows).to_csv(
                OUT / "epl_starting_xi.csv", index=False
            )
            pd.DataFrame(coverage).to_csv(
                OUT / "epl_lineup_coverage.csv", index=False
            )
            pd.DataFrame(matches_out).to_csv(
                OUT / "epl_matches.csv", index=False
            )
            print(
                f"{n}/{len(matches)} | total complete: "
                f"{sum(x['CompleteXI'] for x in coverage)}/{len(coverage)}",
                flush=True,
            )
...
if len(mat) != 1900:
    print(
        f"WARNING: expected 1900 matches, got {len(mat)}",
        file=sys.stderr,
    )
if len(bad):
    print(
        f"WARNING: {len(bad)} matches do not have complete 11+11 lineups.",
        file=sys.stderr,
    )
